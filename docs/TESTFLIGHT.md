...
2. 在开发者门户注册四目标标识，将四个 App ID 与共享 App Group 关联并保存。
...
先修改 `apple/project.yml` 的 CURRENT_PROJECT_VERSION，确保高于自己已上传的构建。脚本会生成默认四目标工程，并将团队、Bundle 前缀和共享组作为归档参数传入。
...
第一条复用与本地私钥匹配的分发证书，选择或创建本项目描述文件，不撤销其他证书。第二条归档、导出并验证四个嵌套包的版本、签名、权限与隐私清单，默认不上传。
...
这套脚本校验四个目标的共享组与 Keychain 权限，同时验证手机主 App 保留原私有 Keychain 组用于升级迁移。
