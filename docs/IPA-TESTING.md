...
无签名归档可用 `scripts/package_unsigned_ipa.py <archive> <output.ipa>` 打包供研究，但它不是可直接运行的分发包。工具验证四个嵌套包和平台，并生成 SHA-256 与结构清单；不会读取 Apple 凭据。

任何重新签名工具都必须保留：

- `PlugIns/VibeWatchPhoneWidgets.appex`；
- `Watch/VibeWatchWatch.app`；
- Watch 内 `PlugIns/VibeWatchWidgets.appex`；
- 一致的 Bundle ID、Watch companion 标识、版本和团队；
- 四个目标被描述文件授权的 App Group 与共享 Keychain。
